 ...
 *
 * Groq is ideal for testing/development as it offers:
 * - Free tier with generous limits (5000+ requests/day)
 * - Very fast inference (90+ tokens/second)
 * - Easy swap with other providers (Claude, Gemini, etc.)
 *
 ...
 * @property apiKey The Groq API key for authentication
 */
internal class GroqInsightGenerationService(
    ...
    companion object {
        ...
        // Model to use (Mixtral 8x7B: fast, high quality, free tier friendly)
        // Alternatives: "gemma-7b-it" (fastest), "llama2-70b-4096" (most capable)
        private const val GROQ_MODEL = "mixtral-8x7b-32768"
     ...
     *
     * The process:
     * 1. Build a detailed prompt instructing AI to analyze emotional content
     * 2. Call Groq API with the transcription
     * 3. Parse the JSON response into InsightContent
     * 4. Create an Insight object with metadata
     * 5. Return the complete insight
     *
     * The prompt instructs the AI to provide:
     ...
     * - AI is explicitly instructed NOT to provide medical advice
     * - AI is instructed NOT to recommend therapy or medication
    ...
    override suspend fun generateInsight(
        ...
        try {
            // Build the request to Groq API (OpenAI-compatible format)
            val request = GroqApiRequest(
                model = GROQ_MODEL,
            ...
            // Call Groq API with the request
            ...
            val choicesArray = jsonResponse["choices"]?.jsonArray
                ?: throw Exception("Invalid Groq API response format: missing choices array")
            val firstChoice = choicesArray.firstOrNull()
                ?: throw Exception("Invalid Groq API response format: empty choices array")
            val messageObject = firstChoice.jsonObject["message"]
                ?: throw Exception("Invalid Groq API response format: no message in choice")
            val textContent = messageObject.jsonObject["content"]?.jsonPrimitive?.content
                ?: throw Exception("Invalid Groq API response format: no content in message")

            // Parse the insight data from AI's response
            val insightData = parseAiResponse(textContent)
            ...
            throw Exception("Failed to generate insight from Groq API: ${e.message}", e)
        }
    }
     ...
     *
     * The prompt requests the AI to analyze the transcription and provide structured
...
- Sentiment: Overall tone (POSITIVE if hopeful/growth-oriented, NEGATIVE if distressed/stuck, NEUTRAL if balanced)
    ...
    /**
     * Parses the AI's JSON response into an InsightContent object.
     *
     * The AI returns a JSON object with insight data. This function:
     ...
     */
    private fun parseAiResponse(jsonResponse: String): InsightContent {
        try {
    ...
    /**
     * Request format for Groq API (OpenAI-compatible format).
     *
     * Groq API follows OpenAI's chat completions API format.
     * Documentation: https://console.groq.com/docs/chat-completions
     */
    @Serializable
    private data class GroqApiRequest(
        val model: String,
        val max_tokens: Int,
    ...
    /**
     * Message object in Groq API request.
     */
    @Serializable
...
/**
 * System prompt that defines AI behavior for insight generation.
 *
 * This prompt:
 * - Establishes the AI as an emotional support journaling tool
 * - Instructs it to provide empathetic analysis
