...
fun DumpRecordingScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.sideEffects.collect { effect ->
            when (effect) {
                ...
                is DumpSideEffect.RequestMicrophonePermission -> onRequestPermission()
    ...
    Scaffold(
        ...
        containerColor = SanctuaryPalette.Neutral,
    ) { innerPadding ->
        Column(
            modifier = modifier
                .fillMaxSize()
            ...
            Spacer(modifier = Modifier.height(SanctuaryDimens.space32))

            // Concentric mic icon with timer
            ConcentricMicrophoneIcon(
                isRecording = viewState.isRecording,
                timerText = viewState.timerText,
            )

            Spacer(modifier = Modifier.height(SanctuaryDimens.space24))
            ...
            if (viewState.recordings.isNotEmpty()) {
                ...
                Text(
                    ...
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(bottom = SanctuaryDimens.space8),
                )
                ...
                LazyColumn(
                    ...
                    items(
                        items = viewState.recordings.take(5),
                        key = { it.id },
                    ) { recording ->
                        RecordingListItem(
                            recording = recording,
